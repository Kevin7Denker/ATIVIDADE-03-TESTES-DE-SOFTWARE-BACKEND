...
const quitar = async (req, res) => {
  try {
    ...
    if (!parseResult.success) {
      return res.status(400).json({ message: 'ParÃ¢metro id invÃ¡lido' });
    }

    const multa = await atualizarMulta(undefined, undefined, new Date(), parseResult.data.id);
    return res.status(200).json(multa);
  } catch (error) {
    const status = error.message === 'Multa nÃ£o encontrada' ? 404 : 400;
    return res.status(status).json({ message: error.message });
  }
};
