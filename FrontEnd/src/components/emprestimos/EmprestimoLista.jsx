...
export function EmprestimoLista({ emprestimos, isAdmin, onDevolucao }) {
  if (!emprestimos.length) {
    return <p className="empty-hint">Nenhum emprestimo registrado.</p>;
  }
          ...
          <article key={e.id} className="list-card">
            <div className="list-card__top">
              <div>
                ...
                <p className="list-card__meta">{e.Usuario?.nome ?? `Usuario #${e.usuario_id}`}</p>
              </div>
            ...
            {!ativo && <p className="list-card__foot">Devolvido em {fmt(e.data_devolucao)}</p>}
            {isAdmin && ativo && (
