...
export function MultasPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const multasNormalizadas = (Array.isArray(m) ? m : (m ? [m] : [])).map((multa) => ({
        ...multa,
        tipo: multa.data_multa ? new Date(multa.data_multa).toLocaleDateString('pt-BR') : 'Multa',
      }));
      setMultas(multasNormalizadas);
      setEmprestimos(e);
    } catch (e) {
  ...
  const handleSalvar = async (e) => {
    ...
    const payload = {
      ...
      data_multa: fd.get('data_multa'),
    };

    if (editando && fd.get('pago') === 'on') {
      payload.data_pagamento = editando.data_pagamento || new Date().toISOString();
    }
        ...
        <>
          <div className="list-cards">
            {atual.map(m => {
              ...
              const pago = Boolean(m.data_pagamento);
              ...
              return (
                <div key={m.id} className="list-card">
                  <div className="list-card__top">
                    ...
                    <span className={`badge ${pago ? 'badge--ok' : 'badge--no'}`}>
                      {pago ? 'Pago' : 'Pendente'}
                    </span>
                  </div>
                  ...
                  {m.data_pagamento && (
                    <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', margin: '0.25rem 0' }}>
                       Pago em {new Date(m.data_pagamento).toLocaleDateString('pt-BR')}
                    </p>
                  )}
              ...
              <div className="modal-body form-stack">
                ...
                <div className="form-field">
                  ...
                  <input
                    type="date"
                    name="data_multa"
                    defaultValue={editando?.data_multa?.split('T')[0] || new Date().toISOString().split('T')[0]}
                    required
                  />
                </div>
                <div className="form-field">
                  <label>Tipo de Multa</label>
                  <select name="tipo" defaultValue={editando?.tipo || 'Atraso'}>
                ...
                {editando && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <input type="checkbox" name="pago" id="chk-quitado" defaultChecked={Boolean(editando.data_pagamento)} style={{ width: 'auto' }} />
