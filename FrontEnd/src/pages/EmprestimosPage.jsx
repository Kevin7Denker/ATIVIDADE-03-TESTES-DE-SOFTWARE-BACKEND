...
export function EmprestimosPage() {
  ...
  const livrosComEmprestimoAtivo = new Set(
    lista
      .filter(e => !e.data_devolucao && (!editando || e.id !== editando.id))
      .map(e => e.livro_id)
  );
  const livrosDisponiveis = livros.filter(l => !livrosComEmprestimoAtivo.has(l.id) || (editando && l.id === editando.livro_id));
